...
class Woo_Feed_Admin
{
		...
		if (
			...
			$js_opts = array(
				...
				'ajax'          => array(
					...
					'nonce' => wp_create_nonce('wpf_feed_nonce'),
					'error' => esc_html__(
						'There was an error processing ajax request.',
						'woo-feed'
					),
				),
				'woocommerce'   => array(
			...
			$feed_created    = apply_filters(
				'CTXFEED_filter_securing_input',
				'GET',
				@$_GET['feed_created'],
				'text'
			);
			$feed_updated    = apply_filters(
				'CTXFEED_filter_securing_input',
				'GET',
				@$_GET['feed_updated'],
				'text'
			);
			$feed_imported   = apply_filters(
				'CTXFEED_filter_securing_input',
				'GET',
				@$_GET['feed_imported'],
				'text'
			);
			$feed_regenerate = apply_filters(
				'CTXFEED_filter_securing_input',
				'GET',
	...
	public function woo_feed_plugin_action_links($links)
	{
		...
		$links[] = sprintf(
			'<a href="%s">%s</a>',
			esc_url(admin_url('admin.php?page=webappick-feed-settings')),

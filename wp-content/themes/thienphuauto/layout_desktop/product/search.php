<?php 
    ...
    $scat_id    = $_GET['scat_id'];if(!$scat_id){$scat_id = '';}
    $key_search = $_GET['s'];if(!$key_search){$key_search = '';}
    ...
    //thêm từ khóa tìm kiếm vào query
    if($key_search !== ''){
        $input_list['s'] = $key_search;
    }
...
<!DOCTYPE html>
<html lang="vi-vn">
    <?php get_template_part('layout_desktop/product/header-page-search'); ?>
    ...
    <div class="product-category">
        <div class="container">
            <?php 
                ...
                $search_url = site_url().'?s='.$key_search.'&scat_id='.$scat_id;
            ?>
                ...
                <div class="filter-list d-flex">
                    ...
                    <div class="item-filter">
                        ...
                        <div class="content-filter">
                            <a style="cursor: pointer;<?php if($brand == ''){ echo 'color:#fda430;font-weight:bold;';} ?>" onclick="location.href='?s=<?php echo $key_search ?>&scat_id=<?php echo $scat_id ?>&sort=new&brand='">All</a>
                            <?php foreach($attribute_info_to_get as $row): ?>
                            <a style="cursor: pointer;<?php if($brand == $row->term_id){ echo 'color:#fda430;font-weight:bold;';} ?>" onclick="location.href='?s=<?php echo $key_search ?>&scat_id=<?php echo $scat_id ?>&sort=new&brand=<?php echo $row->term_id; ?>'"><?php echo $row->name ?></a>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <!---loc thuong hieu --->
                    ...
                    <div class="item-filter">
                        ...
                        <div class="content-filter">
                            <a style="cursor: pointer;" onclick="location.href='?s=<?php echo $key_search ?>&scat_id=<?php echo $scat_id ?>&sort=new'">Mới nhất</a>
                            <a style="cursor: pointer;" onclick="location.href='?s=<?php echo $key_search ?>&scat_id=<?php echo $scat_id ?>&sort=price-asc'">Giá tăng dần</a>
                            <a style="cursor: pointer;" onclick="location.href='?s=<?php echo $key_search ?>&scat_id=<?php echo $scat_id ?>&sort=price-desc'">Giá giảm dần</a>
                            <a style="cursor: pointer;" onclick="location.href='?s=<?php echo $key_search ?>&scat_id=<?php echo $scat_id ?>&sort=name'">Tên A->Z</a>
                        </div>
                    </div>
                </div>
                <div class="clearfix"></div>
            </div>
            <h1 class="name-category">Kết quả tìm kiếm:</h1>
            ...
            <div class="paging">
                <?php 
                    ...
                    $add_new_query .= '&s='.$key_search;

                    if($scat_id !== ''){
                        $add_new_query .= '&scat_id='.$scat_id;
                    }
